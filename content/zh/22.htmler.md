...
欢迎使用 Finch Htmler 指南！本指南将引导您完成在 Finch 应用程序中使用 Htmler 的步骤。无论您是经验丰富的开发人员还是刚刚起步，Finch 都提供了一套强大的工具来简化服务器端 Web 应用程序开发。

## 什么是 Htmler？

Htmler 是一个简单的 HTML 生成器。它用于以更易读和可维护的方式生成 HTML。它在 Finch 中用于为视图生成 HTML。

### 在哪里使用 Htmler？

Htmler 可以在任何需要生成 HTML 的地方使用。例如，您可以在控制器中使用它为视图生成 HTML。您还可以在视图中使用它为模板生成 HTML。使用 htmler，您不需要使用任何模板引擎。您可以直接在 dart 代码中生成 HTML 代码。因此它对于小型项目或生成您保存在 dart 代码中的模板的 HTML 代码非常有用。（对于小部件，您必须将模板移动到需要在生产环境中的单独文件中的小部件文件。因此，要制作单文件项目，htmler 是一个不错的选择。）

以下是如何在项目中使用 htmler 的示例：

### 注意：
    您可以在 htmler 代码中添加 jinja 标签。更多线索请参考此类：lib/src/views/htmler.dart `JJ` 类用于在 htmler 代码中添加 jinja 标签。
